...
// Your code goes here...
export function handlePromise2(arg) {
  return Promise.any(arg)
    .then((val) => val)
    .catch((err) => err);
}
...
// Your code goes here...
export function handlePromise3(arg) {
  return Promise.allSettled(arg)
  .then((results) => results.map((result) => result.status && result))
  .catch((err) => err);
}
